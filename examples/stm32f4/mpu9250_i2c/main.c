...
query_device_id(void)
{
  ...
  while (i2c_is_flag_set(I2C1, I2C_FLAG_START_BIT) == 0);

  i2c_flag_clear(I2C1, I2C_FLAG_START_BIT);
  i2c_send_byte(I2C1, (mpu9250_addr << 1) | write_bit);

  while (i2c_is_flag_set(I2C1, I2C_FLAG_ADDRESS) == 0);

  i2c_flag_clear(I2C1, I2C_FLAG_ADDRESS);

  while (i2c_is_flag_set(I2C1, I2C_FLAG_DATA_REG_EMPTY) == 0);

  constexpr u32 reg_addr = 117;
  i2c_send_byte(I2C1, reg_addr);

  while (i2c_is_flag_set(I2C1, I2C_FLAG_DATA_REG_EMPTY) == 0);
  while (i2c_is_flag_set(I2C1, I2C_FLAG_BYTE_TRANSFER_FINISHED) == 0);
  ...
  while (i2c_is_flag_set(I2C1, I2C_FLAG_DATA_REG_NOT_EMPTY) == 0);
