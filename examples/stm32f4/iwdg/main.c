...
rcc_setup(void)
{
  ...
  while (rcc_is_osc_ready(RCC_OSC_LSI) == 0);
}
...
iwdg_setup(void)
{
  ...
  while (iwdg_is_flag_set(IWDG_FLAG_COUNTER_UPDATE) != 0);
  iwdg_set_reload_value(128);

  while (iwdg_is_flag_set(IWDG_FLAG_PRESCALER_UPDATE) != 0);
